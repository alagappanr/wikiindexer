			...
			while (stream.hasNext()) { 
				...
				if (token != null) {
					...
					if(!split_token){
						...
						stream.set(token_array); //move not required because the remove method automagically does this for us.
						stream.next();
					}
				}
//				System.out.println(stream.tokenStream);
	...
	public boolean splitToken(String token){
		Map<String, String> contrac = new HashMap<String, String>();
...
//		System.out.println("Returning without split "+token);
	...
	public String[] RemoveApostropheArray(String token){
		...
		if(updated_token==null){
			updated_token = contrac.get(token.toLowerCase());
			updated_token = updated_token.substring(0, 1)
					.toUpperCase()
